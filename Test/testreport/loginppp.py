...
class LoginCase(unittest.TestCase):
    ...
    def login(self, username, password):
        # 需要测试的网页
        dr.get("http://dev5.woshidai.com/")

        # 需要输入的用户名，变量名和方法中的一致，find_element_by_id('keywords')为抓取到的用户名的输入框，用谷歌或者火狐F12可以抓取到
        dr.find_element_by_id('keywords').send_keys(username)

        # 需要输入的密码，变量名和方法中的一致，find_element_by_id('password')为抓取到的密码的输入框
        dr.find_element_by_id('password').send_keys(password)

        # 点击登陆按钮，find_element_by_css_selector('.btn')为抓取到的登陆按钮，click()为点击事件
        dr.find_element_by_css_selector(".btn").click()  # 点击登陆按钮

        # 定义测试方法，框架中测试方法以test_开头，底下引号中的中文会在报告中显示，利于清楚的知道测试目的

    def test_login_success(self):
        '''''用户名、密码正确'''
        self.login('17855661015', 'wo1111')  # 调用定义的login方法，传入正确用户名和密码
        sleep(3)
...
if __name__ == '__main__':
    ...
    # suite.addTest(LoginCase("test_login_pwd_null"))
    # suite.addTest(LoginCase("test_login_user_error"))
    # suite.addTest(LoginCase("test_login_user_null"))
