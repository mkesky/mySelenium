...
class BeautifulPicture():
    def __init__(self):   #类的初始化操作
        self.headers = {'User-Agent':'Mozilla/5.0 (Windows NT 6.1; Win64; x64) '
                        'AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.146 Safari/537.36'}  #给请求指定一个请求头来模拟Chrome浏览器
        self.web_url = "https://unsplash.com"   #要访问的网页地址
        self.folder_path = "G:\\Pictures"  #设置图片要存放的文件目录
    ...
    def get_pic(self):
        ...
        for a in all_a:   #循环每个标签，获取标签中图片的url并且进行网络请求，最后保存图片
            ...
            width_height_str = img_url[width_pos : height_pos]     #使用Python的切片功能截取高度和宽度参数，后面用来将参数替换掉
    ...
    def request(self,url):      #返回网页的response
        r = requests.get(url,headers = self.headers)    #向目标url地址发送get请求，返回一个response对象。有没有headrers参数都可以
        return r
